...
public class ProductDaoImpl extends DaoImpl implements ProductDao{
	...
	public ArrayList<Object> getItemDetails(String title) {
		...
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.title='" + title + "'", Product.class);
	...
	public void updateProductDetails(ArrayList<Object> product) {
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.title='" + product.get(0).toString() + "'", Product.class);
		List<Product> result = query.getResultList();
	...
	public ArrayList<Object> getByGenre(String genre) {
		...
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.genre='" + genre + "'", Product.class);
		List<Product> result = query.getResultList();
		for(int i=0;i<result.size();i++){
	...
	public ArrayList<Object> getByRating(String rating) {
		product_list = new ArrayList<Object>();
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.rate='" + rating + "'", Product.class);
		List<Product> result = query.getResultList();
		for(int i=0;i<result.size();i++){
			product_list.add(result.get(i).getTitle());
			product_list.add(result.get(i).getGenre());
	...
	public ArrayList<Object> getByYear(int year) {
		product_list = new ArrayList<Object>();
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.year=" + year, Product.class);
		List<Product> result = query.getResultList();
		for(int i=0;i<result.size();i++){
			product_list.add(result.get(i).getTitle());
			product_list.add(result.get(i).getGenre());
	...
	public ArrayList<Object> getByType(String type) {
		product_list = new ArrayList<Object>();
		TypedQuery<Product> query = em.createQuery("SELECT p FROM Product p WHERE p.type='" + type + "'", Product.class);
		List<Product> result = query.getResultList();
		for(int i=0;i<result.size();i++){
			product_list.add(result.get(i).getTitle());
			product_list.add(result.get(i).getGenre());
